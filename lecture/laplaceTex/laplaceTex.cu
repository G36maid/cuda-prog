...
__global__ void laplacian(float* phi_old, float* phi_new, float* C, bool flag,
		cudaTextureObject_t texOld, cudaTextureObject_t texNew)
{
    ...
    if((x == 0) || (x == Nx-1) || (y == 0) || (y == Ny-1) ) {  
      ...
      yp1 = site + Nx;   // y+1
      if(flag) {
        b = tex1Dfetch<float>(texOld, ym1);      // read d_old via texOld
        ...
        b = tex1Dfetch<float>(texNew, ym1);     // read d_new via texNew
...
//    printf("\n");
    ...
    if((CPU==1)||(CPU==2)) {
      ...
      printf("Input time for GPU (plus texture setup): %f (ms) \n",Intime);
      ...
      while ( (error > eps) && (iter < MAX) ) {
        ...
        laplacian<<<blocks,threads,sm>>>(d_old, d_new, d_C, flag, texOld, texNew);
