	  ...
	  float* phiT_old, float* phi0_new, float* C)
{
    ...
    if (x == 0) {     // left boundary of the sub-lattice 
      if (phiL_old != NULL) {       // if it is not the left boundary of the entire lattice
        l = phiL_old[(Lx-1)+y*Lx];  // x-1, going into the left GPU, thus x-1 is replaced by Lx-1
        r = phi0_old[site+1];       // x+1, inside this GPU 
      } 
      else           // if it is the boundary of the entire lattice 
        skip = 1;
    }
    else if (x == Lx-1) {    // right boundary of the sub-lattice
      if(phiR_old != NULL) {   // if it is not the right boundary of the entire lattice
        l = phi0_old[site-1];
        r = phiR_old[y*Lx];
      } 
      else 
    ...
    else {    // not the left bpundary of the sub-lattice
      l = phi0_old[site-1];	// x-1
      r = phi0_old[site+1];	// x+1
    }

    if (y == 0) {              // bottom boundary of the sub-lattice
      if (phiB_old != NULL) {  // if it is not the bottom boundary of the entire lattice 
         b = phiB_old[x+(Ly-1)*Lx]; // y-1, going into the bottom GPU, thus y-1 is replaced by Ly-1
         t = phi0_old[site+Lx];     // y+1, inside this GPU
      } 
      else 
	skip = 1;
    }
    else if (y == Ly-1) {       // top boundary of the sub-lattice 
      if (phiT_old != NULL) {   // if it is not the top boundary of the entire lattice  
        b = phi0_old[site-Lx];  // y-1, inside this GPU
        t = phiT_old[x];        // y+1, going the top GPU, thus y+1 is replaced by y=0
      } 
      else 
        skip = 1;
    }
    else {        // not the bottom boundary of the sub-lattice
      b = phi0_old[site-Lx];    // y-1
      t = phi0_old[site+Lx];    // y+1
    }

    if (skip == 0) {       // all neighbors are inside this GPU
...
int main(void)
{
 ...
 //  Allocate working space for GPUs.
    ...
    d_1 = (float **)malloc(NGPU*sizeof(float *));     // pointer of pointers
