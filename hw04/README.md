...
| :-- | :-- | :-- | :-- | :-- | :-- |
| 32 | 256 | 1.204 | 68.06 | 2.06×10⁻⁷ | Good |
| 64 | 256 | 1.071 | 76.49 | 4.27×10⁻⁷ | Better |
| 128 | 156 | 1.066 | 76.84 | 1.01×10⁻⁶ | Better |
| 256 | 78 | 1.060 | 77.25 | 8.05×10⁻⁷ | Better |
| **512** | **157** | **1.050** | **78.02** | **1.70×10⁻⁷** | **Optimal** |
| 1024 | 39 | 1.064 | 76.99 | 7.00×10⁻⁷ | Good |
...
- **Numerical Accuracy**: 1.70×10⁻⁷ relative error
- **CPU Speedup**: 2.97× (including memory transfers)
...
3. **Shared Memory Utilization**: Efficient use of 512×4 = 2KB shared memory per block
4. **Register Pressure**: Manageable register usage per thread

#### **Grid Size Optimization Pattern**

**Optimal Thread Count per GPU**: ~80,000 threads

- **Block 32**: 256 blocks × 32 = 8,192 threads (underutilized)
- **Block 64**: 256 blocks × 64 = 16,384 threads (good)
- **Block 128**: 156 blocks × 128 = 19,968 threads (better)
- **Block 256**: 78 blocks × 256 = 19,968 threads (better)
- **Block 512**: 157 blocks × 512 = 80,384 threads (optimal)
- **Block 1024**: 39 blocks × 1024 = 39,936 threads (reduced parallelism)
...
- **Small Blocks (32-64)**: Errors ~10⁻⁷ to 10⁻⁶
- **Medium Blocks (128-256)**: Consistent errors ~10⁻⁷
- **Large Blocks (512-1024)**: Best accuracy ~10⁻⁷ to 10⁻⁸
...
- **Total Complexity**: O(N/2 + G×log B) where G is grid size
...
2. **Excellent Accuracy**: 1.70×10⁻⁷ relative error
